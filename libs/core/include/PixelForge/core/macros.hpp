...
#else
#define PF_NOEXCEPT_COND(...) noexcept(__VA_ARGS__)
...
#define PF_REQUIRE_IMPL_EXPR_MSG(expr, msg) pf::require(expr, msg)
...
#define PF_ASSUME(expr)                          \
  do {                                           \
    const bool pf_impl_assume_temp_var = (expr); \
...
private:                                                        \
  type name{default_val};                                       \
                                                                \
public:                                                         \
  [[nodiscard]] constexpr const type& accessor_name() const PF_NOEXCEPT { return name; }
